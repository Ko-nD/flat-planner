...
- ✏️ **Конструктор плана прямо в браузере** — кнопка «➕ Пустой план N×M» создаёт коробку, потом инструменты **🧱 Стена** (клик-клик, Shift — цепочка), **🏠 Комната** (полигон с замыканием), **🚪 Дверь** и **🪟 Окно** (клик возле стены — проём встаёт на ось стены). Стены и комнаты выделяются левым кликом, удаляются `Del`
- 🆕 **Шаблоны квартир** — встроенные (студия / 1-2-3 комн / blank), **свои в localStorage**, импорт/экспорт через JSON
- ↻ **Поворот всего плана** на 90°/180°/270°, привязка к сетке 1/5/10/20 см
...
- 📐 **Линейка** с показом расстояния прямо на плане
...
- [x] Перекомпоновка правой панели: drag-разделитель между «Слои» и «Свойства» + swap
- [x] Размещение проёмов (двери/окна) кликом на стену
- [ ] Drag вершин полигонов комнат и концов стен (inline-редактор)
- [ ] Импорт скана БТИ через AI-промпт (модель возвращает ProjectData JSON)
