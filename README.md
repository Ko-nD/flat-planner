...
- [x] Drag вершин полигонов комнат и концов стен (inline-редактор)
- [x] Импорт скана БТИ через AI-промпт (модель возвращает ProjectData JSON)
- [x] Shareable URL — gzip + base64url проект в hash, без сервера
- [x] Mobile read-only режим (auto при viewport < 820 px / coarse pointer)
