...
- ✏️ **Конструктор плана прямо в браузере** — кнопка «➕ Пустой план N×M» создаёт коробку нужных размеров, потом инструменты **🧱 Стена** (клик-клик, Shift — цепочка) и **🏠 Комната** (полигон с замыканием). Стены и комнаты выделяются левым кликом, удаляются `Del`
...
1. **Возьми готовый шаблон или нарисуй свой** — кнопка **«✨ Новый проект»**:
   - **Создать с нуля**: вводишь ширину и высоту (в метрах) → жмёшь «Создать» → получаешь пустую коробку. Дальше включаешь **🧱 Стена** и **🏠 Комната** в тулбаре и расставляешь геометрию мышью прямо на холсте. Лишние стены/комнаты — клик и `Del`.
...
| `Shift` в режиме 🧱 Стена | После постановки стены — продолжать цепочку от последней точки |
| `Enter` в режиме 🏠 Комната | Закрыть текущий полигон (то же — клик в первую точку или правый клик) |
...
- [x] Свои шаблоны квартир: сохранение в localStorage, экспорт/импорт JSON
- [x] Конструктор плана в UI: пустая коробка W×H + рисование стен и комнат + удаление через `Del`
- [ ] Drag вершин полигонов комнат и концов стен (полноценный inline-редактор)
- [ ] Размещение проёмов (двери/окна) кликом на стену
